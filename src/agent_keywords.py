...
def extract_keywords_llm(text, title):
    prompt = (
        ...
        f"Las palabras clave deben ser términos específicos del ámbito legal y contractual, NO nombres propios, ciudades, números, ni datos personales. "
...
def extract_keywords(inputs: dict) -> dict:
    ...
    for doc in docs:
        title = doc.get('title')
        text = doc.get('text', '')
        keywords = extract_keywords_llm(text, title) if text else []
